...
export function paintTerrain(
	...
	const currentTerrain = extractTerrainData(
		gameMap as unknown as GameMapWithTerrain,
		width * height,
	);
...
function heightToTerrain(
	...
	if (normalizedHeight <= thresholds.ocean) {
		const magnitude =
			Math.floor((normalizedHeight / thresholds.ocean) * 30) + 1;
		return (1 << OCEAN_BIT) | magnitude;
	}
	if (normalizedHeight <= thresholds.plains) {
		const magnitude =
			Math.floor(
				((normalizedHeight - thresholds.ocean) /
					(thresholds.plains - thresholds.ocean)) *
					9,
			) + 1;
		return (1 << LAND_BIT) | magnitude;
	}
	if (normalizedHeight <= thresholds.hills) {
		const magnitude =
			Math.floor(
				((normalizedHeight - thresholds.plains) /
					(thresholds.hills - thresholds.plains)) *
					10,
			) + 10;
		return (1 << LAND_BIT) | magnitude;
	}
	...
	const magnitude =
		Math.floor(
			((normalizedHeight - thresholds.hills) /
				(thresholds.mountain - thresholds.hills)) *
				12,
		) + 20;
...
function extractTerrainData(
	gameMap: GameMapWithTerrain,
	totalTiles: number,
): Uint8Array {
...
export function generateShoresForExport(
	terrainData: Uint8Array,
	width: number,
	height: number,
): Uint8Array {
