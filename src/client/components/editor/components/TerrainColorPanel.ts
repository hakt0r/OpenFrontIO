...
export class TerrainColorPanel extends TailwindElement {
  protected props: Array<EditorStoreKey> = ['isDarkMode', 'isTerrainVisible']
  ...
  render() {
    return html`
      <e-button
        .type=${'secondary'}
        .active=${this.context.isTerrainVisible.value}
        .icon=${'🏕️'}
        @click=${() => (this.context.isTerrainVisible.value = !this.context.isTerrainVisible.value)}
      />
      <div
        class="fixed top-16 right-5 w-[250px] bg-editor-componentBackground border border-editor-border rounded-lg p-4 shadow-lg z-[1000] text-editor-text font-sans ${this.context.isTerrainVisible.value ? '' : 'hidden'}"
      >
        <div>
