...
type SourceType = keyof typeof sourceIcon

@customElement('canvas-overlay')
export class CanvasOverlay extends TailwindElement {
  protected props: Array<EditorStoreKey> = ['mapState', 'transform', 'hoverCoords', 'hoverTerrainInfo']
  ...
  @property({ type: Object }) dataSourceInfo?: { sourceType: SourceType }
    ...
    return html`
      <bezel-panel class=${this.classNames.bezel}>
        ...
        <only-show .if=${this.dataSourceInfo?.sourceType}>
          <overlay-row .label=${sourceIcon[this.dataSourceInfo?.sourceType as keyof typeof sourceIcon]} .value=${sourceLabel[this.dataSourceInfo?.sourceType as keyof typeof sourceLabel]}></overlay-row>
        </only>
