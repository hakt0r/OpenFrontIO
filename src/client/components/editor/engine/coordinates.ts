import type { EditorPosition, EditorTransform } from '../types'

export type { EditorPosition, EditorTransform }

export function canvasToMapCoordinates(canvasX: number, canvasY: number, transform: EditorTransform): EditorPosition {
  return {
...
export function mapToCanvasCoordinates(mapX: number, mapY: number, transform: EditorTransform): EditorPosition {
  return {
...
export function isPositionInBounds(position: EditorPosition, bounds: { width: number; height: number }): boolean {
  return position.x >= 0 && position.x < bounds.width && position.y >= 0 && position.y < bounds.height
}

export function calculateDistance(pos1: EditorPosition, pos2: EditorPosition): number {
