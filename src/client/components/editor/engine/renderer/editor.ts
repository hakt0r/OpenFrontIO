...
export class EditorRenderer extends BaseRenderer {
  ...
  async createMapState(mapName = 'Heightmap'): Promise<MapEditorState | null> {
    ...
    let landTiles = 0
    for (let i = 0; i < terrainData.length; i++) if (terrainData[i] & (1 << 7)) landTiles++
  ...
  async paintBrush(
    ...
    for (const chunkCoord of affectedChunks) {
      ...
      if (result) this.updateChunkInTexture(chunkCoord.chunkX, chunkCoord.chunkY, result)
    }
  ...
  setBrushType(brushType: BrushType): void {
    ...
    if (this.material) this.material.uniforms.u_brushType.value = brushType
  }
  ...
  removeProcessedStrokes(processedCount: number): void {
    ...
    for (let i = remainingStrokeCount * 2; i < strokeCenters.length; i++) strokeCenters[i] = 0
  ...
  canvasToMapCoordinates(canvasX: number, canvasY: number): { x: number; y: number } {
    const canvas = this.canvas as HTMLCanvasElement
    const mapDims = this.chunkManager.mapDimensions
    if (mapDims.width === 0 || mapDims.height === 0) return { x: 0, y: 0 }
